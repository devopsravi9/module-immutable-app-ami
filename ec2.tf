...
resource "null_resource" "ansible" {
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      ...
      "git pull",
      "ansible-playbook robo.yml -e HOST=localhost -e ROLE=${var.COMPONENT} -e ENV=ENV -e DOCDB_ENDPOINT=DOCDB_ENDPOINT -e REDDIS_ENDPOINT=REDDIS_ENDPOINT  -e MYSQL_ENDPOINT=MYSQL_ENDPOINT e DOCDB_USER=DOCDB_USER -e DOCDB_PASS=DOCDB_PASS -e SECRETS=SECRET -e RABBITMQ_USER_PASSWORD=RABBITMQ_USER_PASSWORD",
    ]
  }
}
